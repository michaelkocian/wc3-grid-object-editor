...
// ================================================================
const GRID_COLS = {
  items: ['unam','iabi','iarm','iclb','iclg','iclr','icid','icla','ides','idro','idrp','ifil','igol','ihtp','iicd','iico','ilev','ilum','ilvo','imor','ipaw','iper','ipow','ipri','iprn','isca','isel','issc','isst','isto','istr','iusa','iuse','ubpx','ubpy','ufoo','uhot','isit','ureq','urqa','utip','utub','bdns'],
  abilities: ['anam','aaea','aani','aare','aart','abpx','abpy','abuf','aca1','acac','acap','acas','acat','achd','acdn','adur','aeat','aeff','aefl','aefs','ahdu','aher','ahky','aite','alev','alig','alsk','amac','amat','amcs','amho','amsp','ansf','aord','aorf','aoro','aoru','apri','arac','aran','arar','areq','aret','arhk','arlv','arpx','arpy','arqa','arut','asat','aspt','ata0','ata1','ata2','ata3','ata4','ata5','atac','atar','atat','atp1','auar','aub1','aubx','auby','auhk','ausk','aut1','auu1','urq1','urq2','urq3','urq4','urq5','urq6','urq7','urq8'],
...
export function toggleCategory(type, catId) {
...
// ================================================================
export function loadBinaryFile(type) {
  const inp = document.createElement('input');
  inp.type = 'file';
...
function renderTable(type) {
  ...
  if (!hl) {
    ...
    for (let ri = 0; ri < rows.length; ri++) {
      ...
      const fullName = codeLookup(r.bid);
      ...
      h += '<td class="idc">' + esc(r.bid) + ' (' + esc(fullName) + ')</td>';
    ...
    for (const grp of groups) {
      ...
      const ga = groupAlt ? 'ga' : '';
      const fullName = codeLookup(headRow.bid);
      ...
      h += '<td class="idc">' + esc(headRow.bid) + ' (' + esc(fullName) + ')</td>';
...
// ================================================================
export function exportBinary(type) {
...
export function removeRow(type, idx) {
  const st = S[type]; if (!st) return;
  syncDOM(type);
  if (!confirm('Remove this row? Warcraft3 import can only overwrite existing data. Unless you delete it from your map it will not disappear.')) return;
  const scrollPos = saveScroll(type);
...
export function removeGroup(type, gid) {
  const st = S[type]; if (!st) return;
  syncDOM(type);
  if (!confirm('Remove this object and all its levels? Warcraft3 import can only overwrite existing data. Unless you delete it from your map it will not disappear.')) return;
  const scrollPos = saveScroll(type);
...
export function changeLevelCount(type, gid, rawVal) {
  const st = S[type]; if (!st) return;
  syncDOM(type);
  const scrollPos = saveScroll(type);
...
export function duplicateRow(type, idx) {
  const st = S[type]; if (!st) return;
  syncDOM(type);
...
// ================================================================
export function closePopup(id) { document.getElementById(id).classList.add('hidden'); }
export function esc(s) { return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
export function escA(s) { return String(s).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
